...
import (
	...
	"github.com/wmh11112345/gatt/linux"
)
...
func finish(op byte, h uint16, b []byte) bool {
	...
	e := attEcode(b[4])
	if (b[0] == attOpError) && (e != attEcodeAttrNotFound) {
		fmt.Printf("unexpected protocol error: %s\n", e)
...
func (p *peripheral) DiscoverServices(ds []UUID) ([]*Service, error) {
	...
	for !done {
		...
		default:
			//	str := fmt.Sprintf("%s", attEcode(b[3]))
...
func wrErrHandle(b []byte) error {
	if b[0] != attOpError {
		//fmt.Printf("wrErrHandle: %#v\n", b)
		return nil
	}
	e := attEcode(b[4])
	str := fmt.Sprintf("unexpected wrErrHandle error: %s", e.Error())
	fmt.Println(str)
	return errors.New(str)

}
func (p *peripheral) WriteHandle(handle uint16, value []byte, noRsp bool) (err error) {
	err = nil
...
func (p *peripheral) ReadHandle(handle uint16) (data []byte, err error) {
	err = nil
	b := make([]byte, 3)
...
func (p *peripheral) ReadDescriptor(d *Descriptor) ([]byte, error) {
	...
	if b[0] == 1 {
		...
		if e != attEcodeAttrNotFound {
			fmt.Printf("unexpected ReadDescriptor error: %s", e)
		}
	}
	b = b[1:]
	// TODO: error handling
